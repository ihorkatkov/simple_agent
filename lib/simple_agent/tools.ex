defmodule SimpleAgent.Tools do
  ...
  @read_file %{
    ...
    function: &SimpleAgent.Tools.read_file/2
  }
  ...
  @list_files %{
    ...
    function: &SimpleAgent.Tools.list_files/2
  }
  ...
  @edit_file %{
    ...
    function: &SimpleAgent.Tools.edit_file/2
  }

  @doc "Expose a list of all tools (with both metadata & function)"
  def tool_definitions do
    local_tools = [
      Function.new!(@list_files),
      Function.new!(@read_file),
      Function.new!(@edit_file)
    ]
      ...
      Enum.map(response.result["tools"], fn tool ->
        Function.new!(%{
          ...
          function: fn args, _context ->
            SimpleAgent.Tools.mcp_tool_function(tool["name"], args)
          end
        })
      end)
  ...
  def read_file(%{"path" => path}, _context) do
  ...
  def list_files(input, _context) do
  ...
  def edit_file(%{"path" => path, "old_str" => old, "new_str" => new}, _context) do
    case File.read(path) do
      {:ok, content} ->
